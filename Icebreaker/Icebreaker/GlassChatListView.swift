...
struct SimpleIcebreakerChatView: View {
    ...
    private func sendMessage() {
        ...
        chatManager.updateConversation(conversationId: conversation.id, with: text)
    ...
    private func simulateResponse() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            ...
            // Update conversation in chat manager
            chatManager.updateConversation(conversationId: conversation.id, with: response.text)
        }
    }
